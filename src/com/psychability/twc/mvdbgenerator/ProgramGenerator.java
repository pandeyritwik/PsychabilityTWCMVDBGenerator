...
MongoException {
	...
	int port = 27017;
	String db = "TWC";
	String collection = "ProgramMetadata";
	int tuners = 10;
	int programs = 10;
	int stations = 5;
	...
	for (int i = 0; i < tuners; i++) {
		new TunerEventGroupGenerator(mvdbCollection, new Household(
...
class TunerEventGroupGenerato extends Thread {
...
public TunerEventGroupGenerato(DBCollection mvdbCollection, Household tuner,
...
public void run() {
	...
	while (writtenRecords.get() < totalRecords) {
		...
		// Min 5 minutes
		int randomDuration = 300 + rand.nextInt(program
				.getRuntime() - 300);
		entity.setViewDuration(randomDuration);

		// Add viewDuration + up to 5 minutes
		runningCalendar.add(Calendar.SECOND,
				randomDuration + rand.nextInt(300));

		entity.setStation(StationStore.getNextStation());
		entity.setTuner(tuner);

		entity.set_id(String.valueOf(entity.getProgram().getProgramId())
				+ "_" + entity.getStation().getStationId() + "_"
				+ tuner.getHouseholdId() + "_"
				+ String.valueOf(runningCalendar.getTimeInMillis()));
